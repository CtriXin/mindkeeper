...
function detectCli(): string | undefined {
...
function resolveThreadForWrite(args: Record<string, unknown>): {
  thread?: ReturnType<typeof getThreadById>;
...
export function handleLearn(args: Record<string, unknown>, index: BrainIndex): ToolResponse {
  ...
  const files = ((args.files as RecipeFile[]) || []).map((f): RecipeFile => ({
    path: String(f.path),
...
export function handleRecall(args: Record<string, unknown>, index: BrainIndex): ToolResponse {
  ...
  const text = results.map((r, i) => {
    ...
    if (rec.conclusion) { parts.push('', '### 结论', rec.conclusion); }
    if (rec.why) { parts.push('', '### 原因', rec.why); }
    if (rec.when_to_apply) { parts.push('', '### 适用场景', rec.when_to_apply); }
...
export function handleList(args: Record<string, unknown>, index: BrainIndex): ToolResponse {
  let recipes = index.recipes;

  if (args.project) recipes = recipes.filter(r => r.project === String(args.project));
  if (args.tag) {
...
export function handleBootstrap(args: Record<string, unknown>, index: BrainIndex): ToolResponse {
  if (!args.repo) {
...
export function handleCheckpoint(args: Record<string, unknown>): ToolResponse {
...
export function handleFragment(args: Record<string, unknown>): ToolResponse {
  ...
  const allowedKinds: FragmentKind[] = ['dev', 'explore', 'debug', 'fix', 'note'];
  const kind = allowedKinds.includes(rawKind as FragmentKind) ? rawKind as FragmentKind : 'note';
...
export function handleLinkIssue(args: Record<string, unknown>): ToolResponse {
...
export function handleSyncIssue(args: Record<string, unknown>): ToolResponse {
  if (!args.repo) {
...
export function handleThreads(args: Record<string, unknown>): ToolResponse {
  const repo = args.repo ? String(args.repo) : undefined;
...
export function handleBoard(args: Record<string, unknown>): ToolResponse {
  ...
  switch (action) {
    ...
    case 'write': {
      ...
      const board = createBoard((data.project as string) || project, data.repo as string | undefined, data.stale_warning_days as number | undefined);
      board.quadrants = (data.quadrants || board.quadrants) as Board['quadrants'];
      board.memos = (data.memos || board.memos) as Board['memos'];
      saveBoard(board);
...
export function handleCheck(args: Record<string, unknown>, index: BrainIndex): ToolResponse {
