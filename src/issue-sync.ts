...
function ensureIssueLinksDir(): void {
...
function issueLinkPath(rootId: string): string {
  return join(ISSUE_LINKS_DIR, `${rootId}.json`);
}

export function saveIssueLink(record: IssueLinkInput): IssueLinkRecord {
  ensureIssueLinksDir();
  const normalized: IssueLinkRecord = {
...
export function loadIssueLink(rootId: string): IssueLinkRecord | null {
  ensureIssueLinksDir();
  const path = issueLinkPath(rootId);
  if (!existsSync(path)) return null;
  try {
    return JSON.parse(readFileSync(path, 'utf-8')) as IssueLinkRecord;
  } catch {
    return null;
  }
}
...
function issueMdPath(root: string, project: string, issue: string): string {
...
function upsertSection(text: string, title: string, bodyLines: string[]): string {
...
function formatFragmentLine(fragment: FragmentRecord): string {
...
export function syncIssueDigest(thread: ThreadSummary): SyncIssueDigestResult {
  ...
  text = upsertSection(
    ...
    normalFragments.length > 0 ? normalFragments.map(formatFragmentLine) : ['- 暂无 fragment'],
  );
  text = upsertSection(
    text,
    'Mindkeeper Reflections',
    reflections.length > 0 ? reflections.map(formatFragmentLine) : ['- 暂无 reflection'],
  );
...
export function defaultIssueProject(repo: string): string {
