...
/** 检测是否是 pin 请求 */
export function isPinRequest(text: string): boolean {
  return PIN_TRIGGERS.some(re => re.test(text.trim()));
}

/** 解析 pin 请求，提取内容 */
export function parsePinRequest(text: string): { content: string; category?: string } | null {
...
/** 读取 HIGHLIGHTS.md */
export function loadHighlights(): string {
  ensureSceDir();
...
/** 创建默认 HIGHLIGHTS.md */
function createDefaultHighlights(): string {
...
export function appendPin(entry: PinEntry): { success: boolean; error?: string } {
  try {
    ensureSceDir();
...
/** 快捷方法：直接 pin 一段文字 */
export function pin(text: string, options?: {
  source?: string;
  category?: string;
}): { success: boolean; entry?: PinEntry; error?: string } {
...
/** 直接 pin 内容（不需要触发词） */
export function pinDirect(content: string, options?: {
  title?: string;
  source?: string;
  category?: string;
}): { success: boolean; entry?: PinEntry; error?: string } {
