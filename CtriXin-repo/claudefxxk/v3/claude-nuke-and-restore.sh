...
if [ -f "$HOME/.claude.json" ]; then
    ...
    if [ -n "$OLD_USERID" ]; then
        echo "  当前 userID: ${OLD_USERID:0:16}..."
    else
        echo "  ~/.claude.json 存在但未找到 userID"
    fi
else
    echo "  ~/.claude.json 不存在（预期：阶段2已清理）"
fi
