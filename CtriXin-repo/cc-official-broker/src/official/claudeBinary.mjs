...
export function detectDirectConnectSupport(binaryPath) {
  ...
  const helpProbe = runCommand(binaryPath, ["open", "--help"])
  ...
  const openCommandAdvertised =
    helpText.includes("Connect to a Claude Code server") || helpText.includes("open <cc-url>")
  const supported = reliable.length > 0 || openCommandAdvertised
...
export function buildOfficialDoctorReport(config, overrides = {}) {
  ...
  return {
    ...
    current_direction: {
      ...
      official_reuse_now: directConnect?.ok
        ? "direct-connect TUI + headless sdk-url child launch contract + anthropic proxy launch"
        : "anthropic proxy launch + headless sdk-url child launch contract",
      recommended_entry: directConnect?.ok && (auth?.loggedIn || remoteAuth.available)
        ? "official:connect"
      ...
      current_blocker: directConnect?.ok
        ...
        : config.remoteServiceBaseUrl
          ? "this local Claude Code build cannot open broker via direct-connect, so the practical official entry should move to a local Anthropic-compatible proxy backed by the remote service"
