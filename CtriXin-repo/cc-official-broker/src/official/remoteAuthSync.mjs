...
function buildRemoteHost(config) {
  const explicit = normalizeText(process.env.CC_BROKER_REMOTE_CLAUDE_SSH_TARGET || config?.remoteClaudeSshTarget)
  ...
  try {
    ...
    return {
      ...
      meta: {
        ...
        container_name: normalizeText(meta.container_name)
      }
    }
  } catch {
    return null
...
function fetchRemoteFile({ sshTarget, containerName, remotePath, optional = false }) {
  const args = ["-o", "BatchMode=yes", "-o", "ConnectTimeout=8", sshTarget]
  if (normalizeText(containerName)) {
    args.push("docker", "exec", normalizeText(containerName), "cat", remotePath)
  } else {
    args.push("cat", remotePath)
  }
  const result = runCommand("ssh", args, { timeout: 12000 })
...
export function probeRemoteAuthBundle(config) {
  ...
  const rawContainer = process.env.CC_BROKER_REMOTE_CLAUDE_CONTAINER_NAME !== undefined
    ? String(process.env.CC_BROKER_REMOTE_CLAUDE_CONTAINER_NAME)
    : (config?.remoteClaudeContainerName !== undefined ? String(config.remoteClaudeContainerName) : undefined)
  const containerName = rawContainer === undefined ? "claude-code-official" : normalizeText(rawContainer)
  const credentialsPath =
    normalizeText(process.env.CC_BROKER_REMOTE_CLAUDE_CREDENTIALS_PATH || config?.remoteClaudeCredentialsPath) ||
    "/root/.claude/.credentials.json"
  const globalConfigPath =
    normalizeText(process.env.CC_BROKER_REMOTE_CLAUDE_GLOBAL_CONFIG_PATH || config?.remoteClaudeGlobalConfigPath) ||
    "/root/.claude.json"
...
export async function syncRemoteAuthBundle(config) {
  ...
  if (
    cached &&
    cached.meta.ssh_target === sshTarget &&
    cached.meta.container_name === containerName &&
    isCacheFresh(cached) &&
    hasUsableOauth(cached.bundle)
  ) {
    return {
      ok: true,
      auth_dir: authDir,
