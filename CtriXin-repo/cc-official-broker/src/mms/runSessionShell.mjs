...
function printHelp() {
  ...
  printLine("  examples:")
  printLine("    /tool bash echo worker")
  printLine('    /tool write_file {"path":"notes.txt","content":"hello"}')
  printLine('    /tool apply_patch {"path":"notes.txt","search":"hello","replace":"world"}')
  printLine()
}
...
function printBanner({ config, mode, sessionId, runnerKey, projectRoot }) {
  ...
  printLine(`- writable_scope: ${config.runnerWritableScope || "none"}`)
  printLine(`- hint: /help`)
  printLine()
}
