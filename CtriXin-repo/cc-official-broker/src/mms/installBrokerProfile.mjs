...
export function buildPersistentBrokerProfile(config, overrides = {}) {
  ...
  const entryMode = overrides.entryMode || "official_connect"
  ...
  return {
    ...
    remote_service_base_url: overrides.remoteServiceBaseUrl || config.remoteServiceBaseUrl || "http://82.156.121.141:18081",
...
export function buildProfileInstallGuide(config, overrides = {}) {
  const profile = buildPersistentBrokerProfile(config, overrides)
  const entryMode = String(profile.entry_mode || "shell")
  const entryGuide =
    entryMode === "official_connect"
      ? "现在推荐把 profile 默认入口固定为 official_connect，MMS 会先同步远端 claude auth，再直接进入本机 Claude Code UI"
      : "这个 profile 默认走 broker shell，更适合先把本地 worker / 文件工具能力跑稳"
