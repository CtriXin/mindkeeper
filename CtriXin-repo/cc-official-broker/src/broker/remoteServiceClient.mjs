export function buildRemoteServiceHeaders(config, runtimeId = "") {
  const headers = {
  ...
  // Pass runtime_id to upstream so server-side runtime/control plane can identify the target container
  const effectiveRuntimeId = runtimeId || config.remoteServiceRuntimeId || ""
  if (effectiveRuntimeId) {
    headers["x-cc-runtime-id"] = effectiveRuntimeId
  }
...
export async function promptRemoteService({
  ...
  const headers = buildRemoteServiceHeaders(config, runtimeId)
  const metadata = {
    ...toMetadata(routing),
    source
  }

  // Also include runtime_id in metadata for server-side readability
  if (runtimeId) {
    metadata.runtime_id = runtimeId
  }
...
export async function postRemoteAgentEvent({
  config,
  routing,
  eventType,
  source = "cc-official-broker:broker",
  ...
  if (!config?.remoteServiceBaseUrl) {
    return { ok: false, skipped: true, reason: "missing_remote_service" }
  }
  ...
  // Include runtime_id in metadata if provided
  if (runtimeId) {
    body.metadata.runtime_id = runtimeId
  }

  return await fetchRemoteJson(config, buildRemoteServiceUrl(config, "/v1/agent/events"), {
    method: "POST",
    headers: buildRemoteServiceHeaders(config, runtimeId),
    body: JSON.stringify(body)
  })
}

export async function fetchRemoteSessionState({ config, routing, runtimeId = "" }) {
