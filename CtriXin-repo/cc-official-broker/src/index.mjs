...
import { runOfficialProxy } from "./official/runOfficialProxy.mjs"
...
import {
  ...
  listOfficialProxySessionRecords,
...
async function resolveOfficialProxyConfig(baseConfig) {
  const profileId = process.env.CC_BROKER_PROFILE_ID || "official-broker-personal"
  const needsLiveProfile = !baseConfig.remoteServiceBaseUrl ||
    (!baseConfig.remoteServiceBearerToken && !baseConfig.remoteServiceApiKey)
...
if (cmd === "help") {
  printJson({
    commands: {
      ...
      "official:proxy": "launch the real local official claude CLI against a local Anthropic-compatible proxy backed by the configured remote service",
      ...
      "session:history": "print the locally remembered official proxy session history for the current project scope",
...
if (cmd === "official:proxy") {
  try {
    ...
    let argv
    if (printFlagIndex >= 0) {
      argv = [...rest.slice(0, printFlagIndex), "-p", printPrompt]
    } else {
      argv = rest.filter(item => item !== "--print")
    }
    const officialConfig = await resolveOfficialProxyConfig(config)
    const result = await runOfficialProxy(officialConfig, {
      printPrompt,
      argv
    })
    process.exit(result.code)
...
if (cmd === "remote:doctor") {
  ...
  try {
    ...
    process.stderr.write(`  base_url_reachable       ${mark(v.base_url_reachable)}  ${result.remote_service.base_url}\n`)
    process.stderr.write(`  auth_passed              ${mark(v.auth_passed)}  ${result.remote_service.auth_mode}\n`)
    process.stderr.write(`  sticky_two_prompts       ${mark(v.sticky_two_prompts)}  ${result.sticky.remote_session_id_1 || "(none)"}\n`)
    process.stderr.write(`  session_state_consistent ${mark(v.session_state_consistent)}  ${result.sticky.session_state_remote_session_id || "(none)"}\n`)
    if (v.runtime_id_present !== undefined) {
      process.stderr.write(`  runtime_id_present       ${mark(v.runtime_id_present)}\n`)
    }
    const summary = v.overall ? "ACCEPTANCE PASS" : "ACCEPTANCE FAIL"
...
if (cmd === "session:last") {
  try {
    const session = await loadLastSessionRecord(config, {
    ...
    printJson({
      ...
      session,
      official_proxy_history_count: Array.isArray(session?.official_proxy_sessions)
        ? session.official_proxy_sessions.length
        : 0
    })
    process.exit(0)
  } catch (error) {
...
if (cmd === "session:history") {
  try {
    printJson({
      ok: true,
      answer: "loaded local official proxy session history",
      state_path: getLocalSessionRegistryPath(config),
      sessions: await listOfficialProxySessionRecords(config, {
        projectRoot: config.workspaceRoot
      })
    })
    process.exit(0)
