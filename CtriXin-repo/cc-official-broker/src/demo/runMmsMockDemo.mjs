...
import { startMockRemoteService } from "./mockRemoteService.mjs"
...
function buildDemoConfigToml({
  ...
  return [
    ...
    '',
    '[[broker_profiles]]',
    'id = "official-broker-demo"',
    'name = "Official Broker Demo"',
    'enabled = true',
    `broker_base_url = ${tomlString(brokerBaseUrl)}`,
    'device_key_env = "MMS_BROKER_DEVICE_KEY_DEMO"',
    'owner_user_id = "xin"',
    'device_id = "mac"',
    'workspace_id = "personal"',
    'remote_runtime = "official-claude-code"',
    'remote_service_label = "mock-remote-runtime"',
    ...
    `runner_tools = [${(runnerTools || []).map(tomlString).join(", ")}]`,
    `runner_writable_scope = ${tomlString(runnerWritableScope || "none")}`,
...
function buildInlinePythonBootstrap({ configPath, mmsRoot, resumeLast = false }) {
  return [
    ...
    `argv = ['run', 'official-broker-demo'${resumeLast ? ", '--resume-last'" : ""}]`,
...
function resolvePythonBinary() {
  ...
  for (const candidate of ["python3.13", "python3.12", "python3.11", "python3", "python"]) {
    const probe = spawnSync("zsh", ["-lc", `command -v ${candidate}`], {
      encoding: "utf8",
      env: process.env
    })
...
function resolveMmsRoot(repoRoot) {
  const explicit = process.env.MMS_REPO_PATH
  if (explicit) {
    return path.resolve(explicit)
  }

  const candidates = [
    path.resolve(repoRoot, "..", "multi-model-switch"),
    path.resolve("/Users/xin/auto-skills/CtriXin-repo/multi-model-switch")
  ]
...
async function seedRememberedSession({ config, brokerBaseUrl, workspaceRoot, sessionId }) {
  ...
  const createPayload = buildCreateSessionRequest(seedConfig, {
    ...
    initialGoal: "seed remembered session for mock demo",
...
export async function runMmsMockDemo(config, overrides = {}) {
  const workspaceRoot = path.resolve(overrides.projectRoot || config.workspaceRoot || process.cwd())
  const moduleDir = path.dirname(fileURLToPath(import.meta.url))
  const repoRoot = path.resolve(moduleDir, "..", "..")
  const mmsRoot = resolveMmsRoot(repoRoot)
  const python = resolvePythonBinary()

  const tempRoot = path.resolve(repoRoot, "tmp", "mms-mock-demo")
  ...
  const mockRemote = await startMockRemoteService()
  const broker = await startBrokerStub({
    config: {
      ...config,
      deviceKey: config.deviceKey || "demo-device-key",
      remoteServiceBaseUrl: mockRemote.baseUrl,
      remoteServiceBearerToken: "",
      remoteServiceApiKey: "",
      remoteServiceEndpoint: "responses",
      remoteServiceLabel: "mock-remote-runtime",
      requestSource: "cc-official-broker:mms-mock-demo"
    }
  })
  ...
  const configToml = buildDemoConfigToml({
    ...
    remoteServiceBaseUrl: mockRemote.baseUrl,
    workspaceRoot,
    runnerTools: config.runnerTools,
    runnerWritableScope: config.runnerWritableScope
  })
  ...
  const launcherPath = path.join(tempRoot, "run-mms-broker-demo.py")
  await writeFile(
    launcherPath,
  ...
  process.stdout.write("MMS mock demo is ready.\n")
  process.stdout.write(`- mms_config_dir: ${mmsConfigDir}\n`)
  process.stdout.write(`- broker: ${broker.baseUrl}\n`)
  process.stdout.write(`- mock_remote: ${mockRemote.baseUrl}\n`)
  ...
  if (Array.isArray(config.runnerTools) && config.runnerTools.includes("write_file")) {
    process.stdout.write('  /tool write_file {"path":"demo.txt","content":"worker wrote this"}\n')
  }
  ...
  const child = spawn(python, [launcherPath], {
    ...
    env: {
      ...process.env,
      MMS_BROKER_DEVICE_KEY_DEMO: config.deviceKey || "demo-device-key",
  ...
  await mockRemote.close().catch(() => {})

  return {
    ok: (result.code || 0) === 0,
    answer: "mms mock demo finished",
    exit_code: result.code,
    signal: result.signal,
    mms_config_dir: mmsConfigDir,
    local_state_path: localStatePath,
    broker_base_url: broker.baseUrl,
    remote_service_base_url: mockRemote.baseUrl
  }
}
