...
export async function runRemoteDoctor(config, { sessionId = "", promptPrefix = "remote-doctor" } = {}) {
  ...
  const firstTurn = await captureProbe(async () => {
    ...
    return {
      ...
      runtime_id: result.ccMeta?.meta?.runtime_id || ""
    }
  })
  ...
  // Build verdict — explicit pass/fail per Phase 1 acceptance criterion
  const runtimeIdFromProbe = firstTurn.ok ? firstTurn.runtime_id || "" : ""
  report.verdict = {
    base_url_reachable: report.probes.healthz.ok || firstTurn.ok,
    auth_passed: firstTurn.ok,
    sticky_two_prompts: report.sticky.same_remote_session_id && report.sticky.second_turn_reused,
    session_state_consistent: report.probes.session_state.ok &&
      Boolean(remoteSessionIdFromState && remoteSessionId1 === remoteSessionIdFromState),
    runtime_id_present: Boolean(runtimeIdFromProbe),
    runtime_id: runtimeIdFromProbe || null
  }

  report.verdict.overall = report.verdict.base_url_reachable &&
    report.verdict.auth_passed &&
    report.verdict.sticky_two_prompts
