...
### 2.3 从 `CLIProxyAPI` 实验得到的补充结论

这轮从 `cc-mcp-bridge/docs/CLIPROXYAPI_GATEWAY_REFERENCE.md` 又补强了一层判断：

- 本地 client 只连一个统一 `base_url + key` 是对的
- runtime pool / account pool / egress policy 应该藏在 gateway 后面
- 登录链路和运行链路可以拆开，不必混成一个大过程

但它也再次说明一件事：

- 这些结论证明的是“gateway 产品形态可行”
- 不是“完整 official TUI direct-connect 已经解决”
