...
const c = {
    ...
    gray: (s) => isColor ? `\x1b[90m${s}\x1b[0m` : s,
};
...
function groupThreadsByRepo(threads, cwd) {
    ...
    for (const [repo, items] of map) {
        ...
        const group = { repo, name: projName(repo), local: isLocal, threads: items };
        (isLocal ? local : other).push(group);
    }
    // 组内按最新 thread 排序
    const byNewest = (a, b) => b.threads[0].createdAtMs - a.threads[0].createdAtMs;
    local.sort(byNewest);
    other.sort(byNewest);
    return { local, other };
}
function printThreadGroups(groups, limit, dim, detailed = false) {
    if (detailed) {
...
function cmdRecipe() {
    ...
    if (recipe.framework)
        console.log(`框架: ${recipe.framework}`);
    if (recipe.project)
        console.log(`项目: ${recipe.project}`);
...
function cmdThread() {
    ...
    if (sub === 'rm') {
        ...
        try {
            execSync(`dst rm ${id}`, { encoding: 'utf-8' });
        }
        catch {
            console.log(`删除失败: ${id}`);
        }
    ...
    if (sub === 'archive') {
        ...
        try {
            execSync(`dst archive ${id}`, { encoding: 'utf-8' });
        }
        catch {
            console.log(`归档失败: ${id}`);
        }
...
switch (command) {
    ...
    default: {
        // 单个参数时先尝试智能 ID 匹配
        if (args.length === 1 && tryDirectShow(args[0]))
            break;
        cmdSearch(args.join(' '));
        break;
    }
