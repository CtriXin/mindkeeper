...
import { loadBoard, createBoard, addBoardItem, updateBoardItem, addBoardMemo, checkBoards, listBoardSummaries, archiveStaleItems, saveBoard, checkRecipeStaleness, deprecateStaleRecipes, getRecipeHealthSummary, findMatchingBoardItems, storeDigest, recallDigests, invalidateDigest, listDigestSummaries, } from './storage.js';
...
export function handleCheckpoint(args) {
    ...
    const ckptFindings = args.findings || [];
...
// ── Digest Handler ──
export function handleDigest(args) {
    ...
    switch (action) {
        ...
        case 'list': {
            ...
            const text = summaries.map(s => `- \`${s.id}\` ${s.name} [${s.keywords.slice(0, 3).join(', ')}] — 访问 ${s.accessCount} 次 (${s.created.slice(0, 10)})`).join('\n');
...
export function handleSearch(args) {
    ...
    const types = args.types
        ? (Array.isArray(args.types) ? args.types : String(args.types).split(',')).filter((t) => ['thread', 'fragment', 'recipe'].includes(t))
        : undefined;
    const repo = args.repo ? String(args.repo) : undefined;
