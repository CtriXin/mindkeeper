...
export function loadIndex() {
    ...
    if (!existsSync(INDEX_PATH)) {
        const empty = { version: '2.0', updated: new Date().toISOString(), recipes: [], units: [] };
...
function extractSectionText(body, header) {
...
function parseFileEntries(section) {
    if (!section)
        return [];
    return section.split('\n')
        .map(l => l.trim())
        ...
        if (m)
            return { path: m[1].trim(), description: m[2].trim() };
        return { path: text, description: '' };
    });
}
function parseChangelog(section) {
    if (!section)
        return [];
    return section.split('\n')
        .map(l => l.trim())
...
function recipeBodyToMarkdown(recipe) {
    const sections = [];
    // Insight 专用字段
    ...
    const files = asArray(recipe.files);
    const gotchas = asArray(recipe.gotchas);
    const corrections = asArray(recipe.corrections);
    const changelog = asArray(recipe.changelog);
    if (steps.length > 0) {
        sections.push('## 实现步骤');
    ...
    if (files.length > 0) {
        sections.push('## 涉及文件');
        files.forEach(f => sections.push(`- \`${f.path}\` — ${f.description}`));
        sections.push('');
    }
    if (gotchas.length > 0) {
        sections.push('## 已知坑点');
...
export function recipeMetaFrom(recipe) {
    return {
        id: recipe.id,
        type: recipe.type || 'recipe',
        triggers: recipe.triggers,
...
/** 批量更新访问统计 */
export function touchRecipes(index, ids) {
    if (ids.length === 0)
        return;
...
export function metaFromUnit(unit) {
...
export function boardPath(projectSlug) {
    return join(BOARDS_DIR, `${projectSlug}.json`);
}
function archiveBoardPath(projectSlug) {
...
export function listBoardSlugs() {
    ensureBoardsDir();
...
/** 加载指定项目的 board */
export function loadBoard(projectSlug) {
...
/** 创建新 board */
export function createBoard(project, repo, staleWarningDays) {
    ensureBoardsDir();
    const board = {
        project,
        repo,
...
/** 添加条目到 board */
export function addBoardItem(projectSlug, title, quadrant, options) {
...
export function updateBoardItem(projectSlug, itemId, changes) {
    ...
    for (const qk of QUADRANT_KEYS) {
        ...
        if (changes.deadline !== undefined)
            item.deadline = changes.deadline;
...
/** 添加备忘 */
export function addBoardMemo(projectSlug, text, source) {
    const board = loadBoard(projectSlug) || createBoard(projectSlug);
    const memo = {
        text,
        created: new Date().toISOString().slice(0, 10),
...
/** 获取 board 文件修改时间 */
export function getBoardMtime(projectSlug) {
    const path = boardPath(projectSlug);
    if (!existsSync(path))
        return null;
    try {
...
export function archiveStaleItems(projectSlug) {
    const board = loadBoard(projectSlug);
    if (!board)
        return 0;
...
export function checkRecipeStaleness(index) {
    const signals = [];
    const now = Date.now();
...
export function deprecateStaleRecipes(index) {
    const now = Date.now();
    const deprecated = [];
    for (const meta of index.recipes) {
...
/** 获取 recipe 健康摘要 */
export function getRecipeHealthSummary(index) {
    const now = Date.now();
...
export function findMatchingBoardItems(taskText) {
