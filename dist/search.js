...
function parseThreadMeta(content) {
    const meta = {};
...
function scoreWithFieldBoost(fields, keywords) {
    let totalScore = 0;
...
// ── 搜索 Threads ──
function searchThreads(query, repo, limit = 10) {
    ...
    try {
        ...
        for (const file of files) {
            ...
            if (repo && meta.repo && meta.repo !== repo)
                continue;
            const searchText = [
                meta.task || '',
                meta.branch || '',
...
// ── 搜索 Fragments ──
function searchFragments(query, repo, limit = 10) {
    ...
    try {
        ...
        for (const file of files) {
            ...
            for (const line of content.split('\n').filter(Boolean)) {
                let frag;
                try {
                    frag = JSON.parse(line);
                }
                catch {
                    continue;
                }
                if (repo && frag.repo !== repo)
                    continue;
                const { score, matched } = scoreWithFieldBoost([
...
function searchRecipes(query, limit = 5) {
